...
namespace whi_nav2_bt_actions_server
{
	...
	Status LocomotionOffset::onCycleUpdate()
	{
		...
		if (!isCollisionFree(cmd_twist_.twist, currentPose))
		{
			stopRobot();
	...
	bool LocomotionOffset::isCollisionFree(const geometry_msgs::msg::Twist& CmdVel,
		const geometry_msgs::msg::PoseStamped& CurrentPose)
	{
		...
		double dt = 1.0 / cycle_frequency_;
		for (double t = 0.0; t < simulate_ahead_time_; t += dt)
		{
			pose2d.x += CmdVel.linear.x * std::cos(pose2d.theta) * dt;
			pose2d.y += CmdVel.linear.x * std::sin(pose2d.theta) * dt;
			pose2d.theta = angles::normalize_angle(pose2d.theta + CmdVel.angular.z * dt);

			bool fetchCostmap = (t == 0.0);
			if (!collision_checker_->isCollisionFree(pose2d, fetchCostmap))
			{
				return false;
			}
		}
